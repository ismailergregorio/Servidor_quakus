...
public class ControleDeFrota {
    ...
    @GET
    public List<Frota> ListaEntrgas() {
        return repositoriofrota.listAll();
    }

    @GET
    @Path("/{id}")
    public Response ListaEntrgas(@PathParam("id") Integer id) {
        Frota f = repositoriofrota.findById(Long.valueOf("" + id));
        if (f != null)
    ...
    @POST
    @Transactional
    public Response create(Frota frota) {
        try {
